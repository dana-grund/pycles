...
cdef class SpectraStatistics2d:
    # SpectraStatistics computes 3d spectra (u,v,w) at varying z -> profiles integrated over x,y
    # SpectraStatistics2d computes 2d spectra (u,w) at varying z -> profiles integrated overx, averaged over y
    # use case: Straka stable bubble (x,z)
    ...
    def __init__(self, Grid.Grid Gr, PrognosticVariables.PrognosticVariables PV, DiagnosticVariables.DiagnosticVariables DV,
        ...
        NC.create_condstats_group('spectra2d','wavenumber', self.wavenumbers, Gr, Pa)
        ...
        if 'qt' in PV.name_index:
            NC.add_condstat('qt_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)
        if 'theta_rho' in DV.name_index:
            NC.add_condstat('theta_rho_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)
        if 'thetali' in DV.name_index:
            NC.add_condstat('thetali_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)
        if 'theta' in DV.name_index:
            NC.add_condstat('theta_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)
        if 'qt_variance' in DV.name_index:
            NC.add_condstat('qtvar_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)
        if 'qt_variance_clip' in DV.name_index:
            NC.add_condstat('qtvarclip_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)
        if 's_variance' in DV.name_index:
            NC.add_condstat('svar_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)
        if 'covariance' in DV.name_index:
            NC.add_condstat('covar_spectrum', 'spectra2d', 'wavenumber', Gr, Pa)

        if 's' in PV.name_index and 'qt' in PV.name_index:
            NC.add_condstat('s_qt_cospectrum', 'spectra2d', 'wavenumber', Gr, Pa)
    ...
    cpdef stats_io(self, Grid.Grid Gr, ReferenceState.ReferenceState RS, PrognosticVariables.PrognosticVariables PV,
        ...
        spec = np.add(spec_u, spec_w)
        NC.write_condstat('energy_spectrum', 'spectra2d', spec[:,:], Pa)
        NC.write_condstat('energy_spectrum_u', 'spectra2d', spec_u[:,:], Pa)
        NC.write_condstat('energy_spectrum_w', 'spectra2d', spec_w[:,:], Pa)

        if 's' in PV.name_index:
            var_shift = PV.get_varshift(Gr, 's')
            self.fluctuation_forward_transform(Gr, Pa, PV.values[var_shift:var_shift+npg], data_fft_s[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft_s[:])
            NC.write_condstat('s_spectrum', 'spectra2d', spec[:,:], Pa)
        if 'qt' in PV.name_index:
            var_shift = PV.get_varshift(Gr, 'qt')
            self.fluctuation_forward_transform(Gr, Pa, PV.values[var_shift:var_shift+npg], data_fft[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft[:])
            NC.write_condstat('qt_spectrum', 'spectra2d', spec[:,:], Pa)
        if 's' in PV.name_index and 'qt' in PV.name_index:
            spec = self.compute_cospectrum(Gr, Pa, data_fft_s[:], data_fft[:])
            NC.write_condstat('s_qt_cospectrum', 'spectra2d', spec[:,:], Pa)
        ...
        if 'theta_rho' in DV.name_index:
            ...
            self.fluctuation_forward_transform(Gr, Pa, DV.values[var_shift:var_shift+npg], data_fft[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft[:])
            NC.write_condstat('theta_rho_spectrum', 'spectra2d', spec[:,:], Pa)
        if 'thetali' in DV.name_index:
            var_shift = DV.get_varshift(Gr, 'thetali')
            self.fluctuation_forward_transform(Gr, Pa, DV.values[var_shift:var_shift+npg], data_fft[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft[:])
            NC.write_condstat('thetali_spectrum', 'spectra2d', spec[:,:], Pa)
        if 'theta' in DV.name_index:
            var_shift = DV.get_varshift(Gr, 'theta')
            self.fluctuation_forward_transform(Gr, Pa, DV.values[var_shift:var_shift+npg], data_fft[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft[:])
            NC.write_condstat('theta_spectrum', 'spectra2d', spec[:,:], Pa)

        if 'qt_variance' in DV.name_index:
            var_shift = DV.get_varshift(Gr, 'qt_variance')
            self.fluctuation_forward_transform(Gr, Pa, DV.values[var_shift:var_shift+npg], data_fft[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft[:])
            NC.write_condstat('qtvar_spectrum', 'spectra2d', spec[:,:], Pa)

        if 'qt_variance_clip' in DV.name_index:
            var_shift = DV.get_varshift(Gr, 'qt_variance_clip')
            self.fluctuation_forward_transform(Gr, Pa, DV.values[var_shift:var_shift+npg], data_fft[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft[:])
            NC.write_condstat('qtvarclip_spectrum', 'spectra2d', spec[:,:], Pa)

        if 's_variance' in DV.name_index:
            var_shift = DV.get_varshift(Gr, 's_variance')
            self.fluctuation_forward_transform(Gr, Pa, DV.values[var_shift:var_shift+npg], data_fft[:])
            spec = self.compute_spectrum(Gr, Pa,  data_fft[:])
            NC.write_condstat('svar_spectrum', 'spectra2d', spec[:,:], Pa)
